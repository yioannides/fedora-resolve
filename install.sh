...
if ! grep -q 'alias resolve-update=' "$USER_HOME/.bashrc"; then 
    echo 'alias resolve-update="git clone https://github.com/yioannides/fedora-resolve.git && cd fedora-resolve && chmod +x ./install.sh && sudo ./install.sh"' >> "$USER_HOME/.bashrc"
fi
